      ...
      {:else}
        {#each filteredItems as item (item.id)}
          ...
          >
            ...
            <div class="w-32 text-center shrink-0">
              <span class="text-4xl font-black" style={stock === 0 ? 'color:rgba(148,163,184,0.18);' : 'color:#5a9e7a;'}>{stock}</span>
            </div>
      ...
      {:else}
        <div class="px-6 py-6 shrink-0" style="border-bottom:1px solid rgba(99,179,237,0.12);">
          <div class="grid grid-cols-2 gap-3 h-24">
            ...
            <div class="rounded-2xl flex flex-col items-center justify-center gap-1" style="background:rgba(90,158,122,0.07); border:2px solid rgba(90,158,122,0.25);">
              <span class="text-xs font-bold" style="color:rgba(90,158,122,0.7);">총 수량</span>
              <span class="text-5xl font-black" style="color:#5a9e7a;">{totalSelectedQty}</span>
              <span class="text-xs font-bold" style="color:rgba(90,158,122,0.55);">개</span>
            </div>
          </div>
        </div>
      {/if}
