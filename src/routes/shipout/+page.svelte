    ...
    {#if filteredItems.length > 0}
      <div class="h-12 bg-base-200 border-b border-base-300 px-6 shrink-0 flex items-center">
        ...
        <div class="w-32 text-center shrink-0">
          <span class="text-xs font-black text-base-content/40 uppercase tracking-wider">재고</span>
        </div>
        <div class="w-52 text-center shrink-0">
          <span class="text-xs font-black text-base-content/40 uppercase tracking-wider">출고수량</span>
        </div>
        ...
        {#each filteredItems as item (item.id)}
          ...
          <div
            ...
            class="flex items-center min-h-20 px-6 py-3 border-b border-base-200 transition-colors border-l-4
              {isEmpty ? 'opacity-40 cursor-not-allowed border-l-transparent'
                : isSel ? 'bg-primary/5 border-l-primary cursor-pointer'
                : 'hover:bg-base-50 border-l-transparent cursor-pointer'}"
            onclick={() => !isEmpty && toggleItem(item.id)}
            onkeydown={(e) => !isEmpty && e.key === 'Enter' && toggleItem(item.id)}
          >
            <div class="flex-1 min-w-0">
              <p class="text-xl font-bold {isSel ? 'text-primary' : 'text-base-content'}">{item.nickname ?? item.name_ko}</p>
              <p class="text-xs text-base-content/30">{item.categories?.name ?? ''}</p>
            </div>
            <div class="w-32 text-center shrink-0">
              <span class="text-4xl font-black {stock === 0 ? 'text-base-content/20' : 'text-primary'}">{stock}</span>
            </div>
            <div class="w-52 flex items-center justify-center gap-1.5 shrink-0">
              {#if isSel}
                <button class="btn btn-md btn-square text-2xl" onclick={(e) => { e.stopPropagation(); adjustQty(item.id, -1); }}>−</button>
                <button
                  class="min-w-16 px-3 h-12 rounded-lg border-2 border-primary text-primary font-black text-2xl text-center hover:bg-primary/5"
  ...
  <aside class="hidden md:flex flex-col w-2/5 bg-base-100 border-l border-base-200 shrink-0 min-h-0">
    <div class="flex-1 flex flex-col min-h-0">
      {#if selectedEntries.length === 0}
        <div class="flex flex-col items-center justify-center flex-1 gap-4 px-6">
          ...
          <p class="text-base font-black text-base-content/30">품목을 선택하세요</p>
        </div>
      {:else}
    ...
    <div class="px-6 pt-4 pb-5 border-t border-base-200 space-y-3 shrink-0">
      <div class="pb-3 border-b border-base-200">
        <p class="text-xs font-black text-base-content/40 uppercase tracking-wider mb-2">출고 일시</p>
        <button
          type="button"
          class="h-12 px-4 w-full rounded-xl border border-base-300 bg-base-100 text-base font-bold text-base-content hover:bg-base-200 transition-colors text-left"
...
{#if showNumpadModal && editingItemId !== null}
  ...
  <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"
    ...
    aria-label="닫기"
  >
    <div class="bg-base-100 rounded-2xl shadow-2xl w-[480px] overflow-hidden"
      ...
      <div class="p-6 flex flex-col gap-4">
        <div class="grid grid-cols-2 gap-3 h-24">
          ...
          <div class="rounded-2xl border-2 border-primary/60 bg-primary/5 flex flex-col items-center justify-center gap-1">
            ...
            <span class="text-5xl font-black tracking-widest {numpadValue ? 'text-primary' : 'text-base-content/20'}">{numpadValue || '0'}</span>
            {#if numpadClamped}
        ...
        <button type="button"
          class="btn btn-primary w-full h-14 text-xl font-black"
